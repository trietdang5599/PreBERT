...
All operations use one public entry point: `python -m exp_llm`. Its commands
are `rating`, `rating-matrix`, `preprocess`, `semantic`, and `build-dataset`.
...
pip install -r exp_llm/requirements.txt
```
...
The `python -m exp_llm preprocess` command replaces the legacy
...
or evaluative information. It writes JSONL directly and preserves all original
fields while adding or replacing `filteredReviewText` and `overall_new`.

Validate a run without loading the model:

```bash
.venv/bin/python -m exp_llm preprocess \
  data/Small_Toys_and_Games_5.json \
...
The preprocessing ablation is a complete 2x2 comparison:
...
| Adjusted rating only | `pretrained-rating-only` | `reviewText` | `overall_new` |
| Both preprocessing steps | `pretrained-processed` | `filteredReviewText` | `overall_new` |

Pretrained evaluation using `reviewText` and `overall`:

```bash
python3 -m exp_llm rating \
  --dataset data/Small_All_Beauty_5.json \
  --model qwen_3b \
  --mode pretrained
```

Pretrained evaluation using both preprocessed fields (`filteredReviewText` and
`overall_new`):
...
CLI options; defaults come from `llm_settings.py`:

```bash
.venv/bin/python -m exp_llm rating-matrix
```
...
```bash
.venv/bin/python -m exp_llm rating-matrix --dry-run -- \
  --max-test-samples 20
```
...
Each run writes to
`exp_llm/outputs/<dataset>/<model>/<mode>/`:
